...
class MongoDocument(object):
    ...
    def __init__(self, mongo_client, mongo_session):
        ...
        self._session = mongo_session  # Mongo Client Session
    ...
    def save(self):
        ...
        if not self.document_id:
            result = self._collection.insert_one(self._document, session=self._session)
            self.document_id = result.inserted_id
        else:
            self._document = self._collection.find_one_and_replace(
                {"_id": self.document_id}, self._document, session=self._session)
        self.refresh()

...
class Player(MongoDocument):
    ...
    def __init__(self, discord_bot, mongo_client, mongo_session, document_id=None, user=None):
        super(Player, self).__init__(mongo_client, mongo_session)
    ...
    def white_cards(self):
        ...
        """
        try:
            return [WhiteCard(self._client, self._session, _id) \
                    for _id in self._document["white_cards"]]
        except KeyError:
            return None
    ...
    def answers(self):
        ...
        try:
            return [WhiteCard(self._client, self._session, _id) \
                    for _id in self._document["answers"]]
        except KeyError:
            return None
